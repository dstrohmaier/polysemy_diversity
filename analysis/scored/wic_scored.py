...
def _performance_table(pairs: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = [
        _performance_row(grp, k_senses=k, offset=round(off, 4))
        for (k, off), grp in pairs.groupby(["k_senses", "offset"])
    ]
    rows.append(_performance_row(pairs, k_senses="all", offset="all"))
    return pd.DataFrame(rows)


def _plot_performance_vs_offset(perf: pd.DataFrame, figures_dir: Path) -> None:
    """Accuracy and F1 against offset, hued by k, with CI error bars."""
    import matplotlib.pyplot as plt

    cells = perf[perf["offset"] != "all"].copy()
    cells["offset"] = cells["offset"].astype(float)

    specs = [("accuracy", "acc_ci_low", "acc_ci_high", "Accuracy"),
             ("F1", "f1_ci_low", "f1_ci_high", "F1")]
    for value_col, lo_col, hi_col, label in specs:
        fig, ax = plt.subplots()
        for k, grp in cells.sort_values("offset").groupby("k_senses"):
            yerr = [grp[value_col] - grp[lo_col], grp[hi_col] - grp[value_col]]
            ax.errorbar(grp["offset"], grp[value_col], yerr=yerr, marker="o",
                        capsize=3, label=f"k={k}")
        ax.set_xlabel("Zipfian slope offset")
        ax.set_ylabel(label)
        ax.legend(title="senses")
        save_fig(fig, figures_dir, f"wic_{value_col.lower()}_vs_offset")
...
def analyse_wic_scored(scores_dir: Path, sim_dir: Path, out_root: Path) -> None:
    ...
    _plot_performance_vs_offset(perf, figures_dir)
